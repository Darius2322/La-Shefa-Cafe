...
export default async function FaqPage() {
  ...
  return (
    <div className="container-lsc py-10 sm:py-14 max-w-2xl">
      <h1 className="font-display text-display-md sm:text-display-lg text-brown mb-8 sm:mb-10">
        Frequently Asked Questions
      </h1>

      {faqs.length === 0 ? (
        <div className="border border-dashed border-brown/25 rounded-sm p-10 text-center">
          <HelpCircle size={28} strokeWidth={1.5} className="mx-auto mb-3 text-brown/30" />
        ...
        categories.map((cat) => (
          <div key={cat} className="mb-8 sm:mb-10">
            <h2 className="font-display text-lg sm:text-xl text-teal capitalize mb-4">{cat}</h2>
            <div className="space-y-6">
              {faqs
                .filter((f: any) => (f.category || "general") === cat)
                .map((f: any) => (
                  <div key={f.id} className="divider pt-4">
                    <p className="font-medium text-brown mb-1 text-sm sm:text-base">{f.question}</p>
